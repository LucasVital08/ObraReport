...
export default function RelatorioFinalPage() {
  ...
  // Somente acompanhamento quando esta obra é acompanhada como contratante.
  ...
  const [opts, setOpts] = React.useState({
    includeExpenses: viewOnly ? vis.gastos : true,
    includeVideos: true,
    includeInternalOccurrences: viewOnly ? vis.ocorrencias : true,
          ...
          <div className="lg:sticky lg:top-20 space-y-5">
            <Card className="p-5">
              ...
              <div className="space-y-2">
                {!viewOnly && <Toggle label="Incluir gastos" checked={opts.includeExpenses} onChange={(v) => setOpts({ ...opts, includeExpenses: v })} />}
                <Toggle label="Incluir vídeos" checked={opts.includeVideos} onChange={(v) => setOpts({ ...opts, includeVideos: v })} />
                {!viewOnly && <Toggle label="Incluir ocorrências internas" checked={opts.includeInternalOccurrences} onChange={(v) => setOpts({ ...opts, includeInternalOccurrences: v })} />}
